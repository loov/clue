// Package msvc provides MSVC toolchain implementation for Windows.
package msvc

// Installation holds the discovered Visual Studio installation details.
...
// Error represents a structured error for MSVC discovery failures.
type Error struct {
...
// installLink is the URL where users can download Visual Studio
const installLink = "https://visualstudio.microsoft.com/downloads/"

// newNotFoundError creates an error indicating MSVC was not found
// with a helpful message including the download link.
func newNotFoundError() *Error {
	return &Error{
		Type:        "not_found",
		Message:     "MSVC not found. Install Visual Studio: " + installLink,
		InstallLink: installLink,
	}
}

// newVCVarsError creates an error indicating vcvarsall.bat failed.
func newVCVarsError(details string) *Error {
	return &Error{
		Type:        "vcvars_failed",
...
// newToolsNotFoundError creates an error indicating VC tools were not found
// in the Visual Studio installation.
func newToolsNotFoundError(vsPath string) *Error {
	return &Error{
