package build

import (
	"fmt"
	"os"

	"github.com/loov/clue/internal/toolchain"
)

// GCCToolchain implements the Toolchain interface for GCC
type GCCToolchain struct {
	cc     string
	cxx    string
	ar     string
	target toolchain.Platform
}

// CC returns the C compiler path
func (t *GCCToolchain) CC() string {
	return t.cc
}

// CXX returns the C++ compiler path
func (t *GCCToolchain) CXX() string {
	return t.cxx
}

// AR returns the archiver path
func (t *GCCToolchain) AR() string {
	return t.ar
}

// Name returns the toolchain name
func (t *GCCToolchain) Name() string {
	return "gcc"
}

// IsCrossCompiler returns true if configured for cross-compilation
func (t *GCCToolchain) IsCrossCompiler() bool {
	return isCrossCompiler(t.cc)
}

// String returns a descriptive string for build output
func (t *GCCToolchain) String() string {
	if t.IsCrossCompiler() {
		return fmt.Sprintf("%s (cross)", t.cc)
	}
	return "gcc (native)"
}

// CompilerFlags generates GCC-specific compiler flags
func (t *GCCToolchain) CompilerFlags(config Config) []string {
	var flags []string

	// Add optimization flag
	if opt := toolchain.OptimizationFlag(config.Optimize); opt != "" {
		flags = append(flags, opt)
	}

	// Add warning flags
	flags = append(flags, toolchain.WarningFlagsForLevel(config.Warnings)...)

	// Add warnings-as-errors flag
	if config.WarningsAsErrors {
		flags = append(flags, "-Werror")
	}

	// Add debug flag
	if dbg := toolchain.DebugFlag(config.Debug); dbg != "" {
		flags = append(flags, dbg)
	}

	// Add sanitizer flags (GCC doesn't support memory sanitizer)
	if len(config.Sanitizers) > 0 {
		for _, san := range config.Sanitizers {
			if san == "memory" {
				fmt.Fprintf(os.Stderr, "Warning: MemorySanitizer not available on GCC, skipping -fsanitize=memory\n")
				continue
			}
			flags = append(flags, "-fsanitize="+san)
		}
	}

	// Add LTO flag
	if config.LTO {
		flags = append(flags, "-flto")
	}

	// Add PIC flag
	if config.PIC {
		flags = append(flags, "-fPIC")
	}

	// Add coverage flags (GCC gcov-based coverage)
	if config.Coverage {
		flags = append(flags, "-fprofile-arcs", "-ftest-coverage")
	}

	// Append raw compiler flags
	flags = append(flags, config.RawCompiler...)

	return flags
}

// LinkerFlags generates GCC-specific linker flags
func (t *GCCToolchain) LinkerFlags(config Config, sysLibs []string) []string {
	var flags []string

	// Add system library flags
	for _, lib := range sysLibs {
		flags = append(flags, "-l"+lib)
	}

	// Add debug flag (linker may need it for debug symbols)
	if dbg := toolchain.DebugFlag(config.Debug); dbg != "" {
		flags = append(flags, dbg)
	}

	// Add sanitizer flags (linker must match compiler)
	if len(config.Sanitizers) > 0 {
		for _, san := range config.Sanitizers {
			if san == "memory" {
				continue // Skip, already warned at compile time
			}
			flags = append(flags, "-fsanitize="+san)
		}
	}

	// Add LTO flag (linker must match compiler)
	if config.LTO {
		flags = append(flags, "-flto")
	}

	// GCC links coverage automatically via -lgcov (no extra flag needed)

	// Append raw linker flags
	flags = append(flags, config.RawLinker...)

	return flags
}

// Identity returns the compiler identity for cache keys
func (t *GCCToolchain) Identity() (CompilerIdentity, error) {
	return GetCompilerIdentity(t.cc)
}
