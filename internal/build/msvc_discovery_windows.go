...
func FindMSVC() (*MSVCInstallation, error) {
	...
	if _, err := os.Stat(vswherePath); os.IsNotExist(err) {
		return nil, newMSVCNotFoundError()
	}
...
func findMSVCAtPath(vsPath string, version ...string) (*MSVCInstallation, error) {
...
func findVCToolsPath(vsPath string) (string, error) {
	...
	if err != nil {
		return "", newMSVCToolsNotFoundError(vsPath)
	}
...
func captureVCVarsEnvironment(vsPath, arch string) (map[string]string, error) {
	...
	if _, err := os.Stat(vcvarsPath); os.IsNotExist(err) {
		return nil, newMSVCVCVarsError("vcvarsall.bat not found at: " + vcvarsPath)
	}
...
`, vcvarsPath, arch)
	...
	if err != nil {
		return nil, newMSVCVCVarsError("failed to create temp file: " + err.Error())
	}
	...
	if _, err := tmpFile.WriteString(script); err != nil {
		tmpFile.Close()
		return nil, newMSVCVCVarsError("failed to write temp file: " + err.Error())
	}
	tmpFile.Close()
	...
	if err != nil {
		...
		if ok {
			return nil, newMSVCVCVarsError(fmt.Sprintf("exit code %d", exitErr.ExitCode()))
		}
		return nil, newMSVCVCVarsError(err.Error())
	}
	...
	if _, ok := env["PATH"]; !ok {
		return nil, newMSVCVCVarsError("failed to capture PATH from vcvarsall.bat")
	}
